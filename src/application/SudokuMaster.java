...
public class SudokuMaster {
	...
	private boolean checkFullBoard(){
		...
		for(int i = 0;i<9;++i){//Does all columns
		...
		if(valid){
			...
			for(int i = 0;i<9;++i){//Does all rows
			...
			if(valid){
				for(int i = 0;i<9;++i){//Does all boxes
	...
	private boolean checkArrSolution(TextField[] arr){//Given an array it will make sure there is 1-9 with no repeats and none missing
